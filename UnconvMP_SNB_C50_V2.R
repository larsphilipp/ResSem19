...
currentSMIM  <- allPeriodsC5(allData[currentSMIMColumns], "SMIMdir", sampleSize)
currentSPIEX <- allPeriodsC5(allData[currentSPIEXColumns], "SPIEXdir", sampleSize)

# Pattern to describe next Week's CHFEUR
forecastSMI   <- allPeriodsC5(allData[forecastSMIColumns], "SMInext", sampleSize)
forecastSMIM  <- allPeriodsC5(allData[forecastSMIMColumns], "SMIMnext", sampleSize)
forecastSPIEX <- allPeriodsC5(allData[forecastSPIEXColumns], "SPIEXnext", sampleSize)

# Pattern to describe next Week's CHFEUR
currentFxSMI <- allPeriodsC5(allData[currentFxSMIColumns], "CHFEURdir", sampleSize)
currentFxSMIM  <- allPeriodsC5(allData[currentFxSMIMColumns], "CHFEURdir", sampleSize)
currentFxSPIEX <- allPeriodsC5(allData[currentFxSPIEXColumns], "CHFEURdir", sampleSize)

# Pattern to describe next Week's CHFEUR
forecastFxSMI   <- allPeriodsC5(allData[forecastFxSMIColumns], "CHFEURnext", sampleSize)
forecastFxSMIM  <- allPeriodsC5(allData[forecastFxSMIMColumns], "CHFEURnext", sampleSize)
forecastFxSPIEX <- allPeriodsC5(allData[forecastFxSPIEXColumns], "CHFEURnext", sampleSize)


## Outputs
outputPrint(currentSMI)
outputPrint(currentSMIM)
outputPrint(currentSPIEX)

outputPrint(forecastSMI)
outputPrint(forecastSMIM)
outputPrint(forecastSPIEX)

outputPrint(currentFxSMI)
outputPrint(currentFxSMIM)
outputPrint(currentFxSPIEX)
